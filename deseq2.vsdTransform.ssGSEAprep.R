...
#
# cat counts.txt | Rscript deseq2.vsdTransform.ssGSEAprep.r
#
...
vsd.df <- assay(vsd)
vsd.df.new = data.frame("NAME"=rownames(vsd.df),"DESCRIPTION"="na",vsd.df)

write.table(vsd.df.new, file=paste0(args,"_","vst_matrix_ssGSEA_ready_deseq2.gct"), sep="\t",  row.name=FALSE, col.names=TRUE,quote=FALSE)
